...
contract ElementBridge is IDefiBridge {
    ...
    function deriveTranche(address _position, uint256 _expiration) internal view returns (address) {
    ...
    function hashAssetAndExpiry(address asset, uint256 expiry) internal pure returns (uint256) {
    ...
    function interactionCanBeFinalised(Interaction storage interaction) internal view returns (bool, string memory) {
        ITranche tranche = ITranche(interaction.trancheAddress);
