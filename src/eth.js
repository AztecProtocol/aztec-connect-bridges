  ...
  "Object.<anonymous>": function (
    ...
    class EthModule {
      constructor(
        _common,
        _node,
        _throwOnTransactionFailures,
        _throwOnCallFailures,
        _logger,
        _experimentalHardhatNetworkMessageTraceHooks = []
      ) {
      ...
      async processRequest(method, params = []) {
        switch (method) {
          ...
          case "eth_getFilterLogs":
            return this._getFilterLogsAction(
              ...this._getFilterLogsParams(params)
            );
          case "eth_getLogs":
          ...
          case "eth_getStorageAt":
            return this._getStorageAtAction(
              ...this._getStorageAtParams(params)
            );
      ...
      // eth_call
      _callParams(params) {
        return validation_1.validateParams(
          params,
          callRequest_1.rpcCallRequest,
      ...
      async _estimateGasAction(callRequest, blockTag) {
        ...
        const callParams = await this._rpcCallRequestToNodeCallParams(
          callRequest
        );
      ...
      async _getBalanceAction(address, blockTag) {
        const blockNumberOrPending = await this._resolveNewBlockTag(blockTag);
        return base_types_1.numberToRpcQuantity(
          await this._node.getAccountBalance(
            new ethereumjs_util_1.Address(address),
            blockNumberOrPending
          )
        );
      }
      ...
      async _getBlockByHashAction(hash, includeTransactions) {
        ...
        return output_1.getRpcBlock(
          block,
          totalDifficulty,
          output_1.shouldShowTransactionTypeForHardfork(this._common),
          includeTransactions
        );
      }
      ...
      async _getCodeAction(address, blockTag) {
        const blockNumberOrPending = await this._resolveNewBlockTag(blockTag);
        return base_types_1.bufferToRpcData(
          await this._node.getCode(
            new ethereumjs_util_1.Address(address),
            blockNumberOrPending
          )
        );
      ...
      _getLogsParams(params) {
        return validation_1.validateParams(
          params,
          filterRequest_1.rpcFilterRequest
        );
      }
      ...
      async _getTransactionByBlockHashAndIndexAction(hash, index) {
        ...
        return output_1.getRpcTransaction(
          tx,
          output_1.shouldShowTransactionTypeForHardfork(this._common),
          block,
          i
      ...
      async _sendTransactionAction(transactionRequest) {
        ...
        const txParams =
          await this._rpcTransactionRequestToNodeTransactionParams(
            transactionRequest
          );
      ...
      async _signAction(address, data) {
        ...
        return ethereumjs_util_1.toRpcSig(
          signature.v,
          signature.r,
          signature.s
        );
      ...
      async _signTypedDataV4Action(address, typedData) {
        ...
        return this._node.signTypedDataV4(
          new ethereumjs_util_1.Address(address),
          typedMessage
        );
      }
      ...
      _subscribeParams(params) {
        ...
        return validation_1.validateParams(
          params,
          subscribeRequest_1.rpcSubscribeRequest,
          filterRequest_1.optionalRpcFilterRequest
        );
      }
      async _subscribeAction(subscribeRequest, optionalFilterRequest) {
        switch (subscribeRequest) {
          case "newHeads":
            return base_types_1.numberToRpcQuantity(
              await this._node.newBlockFilter(true)
            );
      ...
      _feeHistoryParams(params) {
        const [blockCount, newestBlock, rewardPercentiles] =
          validation_1.validateParams(
            params,
            base_types_1.rpcQuantity,
            blockTag_1.rpcNewBlockTag,
            io_ts_1.optional(t.array(base_types_1.rpcFloat))
          );
        if (blockCount.ltn(1)) {
          throw new errors_1.InvalidInputError(
      ...
      async _feeHistoryAction(blockCount, newestBlock, rewardPercentiles) {
        ...
        const feeHistory = await this._node.getFeeHistory(
          blockCount,
          resolvedNewestBlock,
          rewardPercentiles !== null && rewardPercentiles !== void 0
            ? rewardPercentiles
            : []
        );
        const oldestBlock = base_types_1.numberToRpcQuantity(
          feeHistory.oldestBlock
        );
        const baseFeePerGas = feeHistory.baseFeePerGas.map(
          base_types_1.numberToRpcQuantity
        );
        const gasUsedRatio = feeHistory.gasUsedRatio;
        const reward =
          (_a = feeHistory.reward) === null || _a === void 0
            ? void 0
            : _a.map((rs) => rs.map(base_types_1.numberToRpcQuantity));
        return {
          oldestBlock,
          baseFeePerGas,
          gasUsedRatio,
      ...
      async _rpcCallRequestToNodeCallParams(rpcCall) {
        return {
          ...
          data:
            rpcCall.data !== undefined
              ? rpcCall.data
              : ethereumjs_util_1.toBuffer([]),
          gasLimit:
            rpcCall.gas !== undefined
              ? rpcCall.gas
              : this._node.getBlockGasLimit(),
          value:
            rpcCall.value !== undefined
              ? rpcCall.value
      ...
      async _rpcTransactionRequestToNodeTransactionParams(rpcTx) {
        ...
        const baseParams = {
          ...
          value:
            rpcTx.value !== undefined
              ? rpcTx.value
              : new ethereumjs_util_1.BN(0),
          data:
            rpcTx.data !== undefined
              ? rpcTx.data
              : ethereumjs_util_1.toBuffer([]),
          nonce:
            rpcTx.nonce !== undefined
              ...
              : await this._node.getAccountNextPendingNonce(
                  new ethereumjs_util_1.Address(rpcTx.from)
                ),
        };
        if (
          ...
          if (rpcTx.maxPriorityFeePerGas === undefined) {
            rpcTx.maxPriorityFeePerGas =
              await this._node.getMaxPriorityFeePerGas();
          ...
          if (rpcTx.maxFeePerGas === undefined) {
            ...
            assertions_1.assertHardhatNetworkInvariant(
          ...
          return Object.assign(Object.assign({}, baseParams), {
        ...
        const gasPrice =
          (_a = rpcTx.gasPrice) !== null && _a !== void 0
            ? _a
      ...
      async _handleMineBlockResults(results, sentTx) {
        ...
        if (singleTransactionMined) {
          ...
          const txGasUsed =
            results[0].blockResult.results[0].gasUsed.toNumber();
          ...
          const [sentTxResult, sentTxIndex] =
            this._getTransactionResultAndIndex(sentTx, results);
          const sentTxTrace = sentTxResult.traces[sentTxIndex];
          if (!singleTransactionMined) {
            const blockNumber = sentTxResult.block.header.number;
            const code = await this._node.getCodeFromTrace(
              sentTxTrace.trace,
              new ethereumjs_util_1.BN(blockNumber)
            );
      ...
      async _logSingleTransaction(tx, block, txGasUsed, txTrace) {
        const code = await this._node.getCodeFromTrace(
          txTrace.trace,
          new ethereumjs_util_1.BN(block.header.number)
        );
