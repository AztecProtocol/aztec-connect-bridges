...
contract Working is BridgeTestBase {
    ...
    function setUp() public {
        ...
        //console.log(address(asyncBridge));

        vm.startPrank(MULTI_SIG);
        ROLLUP_PROCESSOR.setSupportedBridge(address(asyncBridge), 200000);
    ...
    function testAsyncLimitOrderKeeper(uint256 _deposit) public {
        ...
        _setTokenBalance(address(DAI), address(asyncBridge), depositAmount, 2);

        uint64 data = _packData(uint160(80), int24(20), int24(20));
        ...
        _lowerPriceToTickRange();

        (, , uint256 amount0, uint256 amount1, , , , , ) = asyncBridge.getDeposit(1);

        console.log("trigger");
        AztecKeeper keeper = new AztecKeeper(1, FACTORY, address(ROLLUP_PROCESSOR), address(asyncBridge));
    ...
    function testAsyncLimitOrder(uint256 _deposit) public {
        vm.assume(_deposit >= 1000000000000000);
        vm.assume(_deposit < 35843821557804033613092184128);
        uint256 depositAmount = _deposit;
        _setTokenBalance(address(DAI), address(asyncBridge), depositAmount, 2);
        //_setTokenBalance(address(DAI), address(ROLLUP_PROCESSOR), depositAmount, 2);
        uint64 data = _packData(uint160(80), int24(20), int24(20));
    ...
    function testAsyncLimitOrderThenCancel(uint256 _deposit) public {
        ...
        vm.assume(_deposit < 2243451767686882001923679787); //maxLiquidity is reached here, causing LO error code.
        ...
        uint64 data = _packData(uint160(50), int24(2), int24(2));
        ...
        /*
        {
            //vm.startPrank(address(asyncBridge) );
    ...
    ) internal returns (uint64 data) {
        (uint160 price, int24 currentTick, , , , , ) = IUniswapV3Pool(POOL).slot0();
        price = (price * _priceDiff) / 100; //decrease price by 50%
        currentTick = TickMath.getTickAtSqrtRatio(price); //get new current tick
        //buy limit
        (int24 _a, int24 _b) = _adjustTickParams(
            currentTick - (_tickLowerDelta * IUniswapV3Pool(POOL).tickSpacing()),
            currentTick + (_tickUpperDelta * IUniswapV3Pool(POOL).tickSpacing()),
            POOL
        );
        uint24 a = uint24(_a);
        uint24 b = uint24(_b);
    ...
    function _lowerPriceToTickRange() internal {
