...
import {UniRangeOrderBridge} from "../../../bridges/uniswapv3/UniRangeOrderBridge.sol";
...
contract UniRange is BridgeTestBase {
    ...
    function _testAsyncLimitOrderThenCancel(
        ...
        /*
        {
            ...
            uint256 bridgeId = encodeBridgeId(
                ...
                emptyAsset,
                getRealAztecAsset(a            dataddress(_tokenB)),
                getRealAztecAsset(address(_tokenA)),
                data
            );
    ...
    function _decimals(address _token) internal returns (uint256) {
        (bool success, bytes memory result) = _token.call(abi.encodeWithSignature("decimals()"));
        if (!success) revert("DECIMALS() FAILED");
    ...
    function _token0Price(
        ...
        uint256 scalar = (10**_decimals(token0)) / (10**_decimals(token1));
