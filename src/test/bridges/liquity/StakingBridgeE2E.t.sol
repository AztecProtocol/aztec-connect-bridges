...
// StakingBridge-specific imports
...
contract StakingBridgeE2ETest is BridgeTestBase {
    ...
    function setUp() public {
        ...
        vm.label(address(bridge.STAKING_CONTRACT()), "LQTYStakingContract");

        // Impersonate the multi-sig to add a new bridge
        vm.startPrank(MULTI_SIG);

        // List the staking-bridge with a gasLimit of 100K
    ...
    function testFullDepositWithdrawalE2E(uint96 _depositAmount) public {
        ...
        bytes32 eventSig = keccak256("DefiBridgeProcessed(uint256,uint256,uint256,uint256,uint256,bool,bytes)");
