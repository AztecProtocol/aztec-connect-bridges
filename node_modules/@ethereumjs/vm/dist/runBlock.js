...
async function applyTransactions(block, opts) {
    ...
    for (let txIdx = 0; txIdx < block.transactions.length; txIdx++) {
        ...
        await receiptTrie.put((0, rlp_1.encode)(txIdx), encodedReceipt);
    }
    return {
        bloom,
        gasUsed,
...
function encodeReceipt(tx, receipt) {
    const encoded = (0, rlp_1.encode)(Object.values(receipt));
...
async function generateTxReceipt(tx, txRes, blockGasUsed) {
    ...
    let txReceipt;
    let encodedReceipt;
    let receiptLog = `Generate tx receipt transactionType=${tx.type} gasUsed=${blockGasUsed.toString()} bitvector=${(0, opcodes_1.short)(abstractTxReceipt.bitvector)} (${abstractTxReceipt.bitvector.length} bytes) logs=${abstractTxReceipt.logs.length}`;
    if (!tx.supports(999)) {
        // Legacy transaction
...
async function _genTxTrie(block) {
    ...
    for (const [i, tx] of block.transactions.entries()) {
        await trie.put((0, rlp_1.encode)(i), tx.serialize());
    }
    return trie.root;
}
