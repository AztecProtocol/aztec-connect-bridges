...
class DefaultStateManager {
    ...
    async getContractStorage(address, key) {
        ...
        const decoded = (0, rlp_1.decode)(value);
        return decoded;
    }
    /**
    ...
    async putContractStorage(address, key, value) {
        ...
        await this._modifyContractStorage(address, async (storageTrie, done) => {
            if (value && value.length) {
                // format input
                const encodedValue = (0, rlp_1.encode)(value);
                if (this.DEBUG) {
    ...
    async generateGenesis(initState) {
        ...
        for (const address of addresses) {
            const balance = new ethereumjs_util_1.BN((0, ethereumjs_util_1.toBuffer)(initState[address]));
            const account = ethereumjs_util_1.Account.fromAccountData({ balance });
            const addressBuffer = (0, ethereumjs_util_1.toBuffer)(address);
